<template>
    <div>
      <!-- 如果没有登录就显示这条提示 -->
      <el-link class="warning-offline" v-if="useronline===0" type="primary" @click="goLogin">您还未登录，点此跳转！</el-link>
      <el-link class="warning-offline" style="width:950px" v-if="ifbelongs===0 && useronline===1" type="primary" @click="goSLPGroup">您不是小组成员，请创建或加入一个小组！</el-link>
      
      <!-- 如果登录就正常显示卡片 -->
      <el-card v-if="useronline===1 && ifbelongs!=0">
  ...
  export default {
    ...
    methods: {
      ...
      async getBlogs() {
        ...
        const { data: res } = await this.$http.get('getGroupslp.php', {
        ...
        this.ifbelongs = res[0]['ifbelongs']
