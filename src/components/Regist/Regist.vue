      ...
      <div class="login_box">
        ...
        <el-form ref="loginFormRef" :model="loginForm" :rules="loginFormRules" label-width="0px" class="login_form">
          ...
          <el-form-item class="btns">
            ...
            <el-link style="margin-left: 60px;" type="primary" @click="$router.push('./Login')">已经注册？</el-link>
          </el-form-item>
        </el-form>
      </div>
    </div>
    ...
    methods: {
      ...
      login() {
        this.$refs.loginFormRef.validate(valid => {
          if (!valid) return
          this.$http.post('http://api-vueblog.czc.cool/regist.php', this.loginForm).then(res => {
            if (res.data.result.status == 1){
                ...
                return this.$message.error('注册失败，错误代号：0')
            }else if(res.data.result.status == 2){
                return this.$message.error('注册失败，此用户名已被占用')
            }else{
                return this.$message.error('注册失败，未知错误')
