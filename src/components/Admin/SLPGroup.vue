<template>
    <div>
      ...
      <el-card>
        ...
        <el-row :gutter="10">
          <el-col :span="8">
            <el-alert username="用户您还未加入小组，请创建小组或通过邀请链接加入小组" v-if="groupMember==='false'" type="warning"></el-alert>
            <!-- <p>当前用户名字：{{queryInfo.username}}</p> -->
            <p v-if="groupMember==='true'">当前小组组长：{{ groupOwnerName }}</p>
              <el-button slot="append" icon="el-icon-search" @click="getGroupList(0)"></el-button>
          </el-col>
          <el-col :span="4">
            <el-button type="primary" v-if="groupMember==='false'" @click="show">创建小组</el-button>
            <el-button type="primary" v-if="groupMember==='true'" @click="show">退出小组</el-button>
          </el-col>
        </el-row>
      </el-card>
      <!-- 表格区域 -->
      <el-table :data="blogList" stripe border>
        ...
        <el-table-column prop="username" label="成员用户名" width="300px"> </el-table-column>
    <!--<el-table-column prop="username" label="文章标题"> </el-table-column>
        ...
        <el-table-column v-if="groupOwner==='true'" label="操作" width="130" fixed="right">
          <template slot-scope="scope">
            <!-- <el-button type="primary" size="mini" icon="el-icon-edit" @click="showEditDialog(scope.row)"></el-button> -->
            <el-button type="danger" size="mini" icon="el-icon-delete" @click="deleteBlog(scope.row.id, scope.row.username)"></el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getGroupList(number,username) {
        // 当number为0时, 跳到第一页
        if (number === 0) this.queryInfo.pagenum = 0
        this.$http.get('http://api-vueblog.czc.cool/slpGroup.php', { params: this.queryInfo}).then((res) => {
          const { data } = res || {}
          if (!Array.isArray(data)) return this.$message.error('获取数据失败');
          this.blogList = data;
          this.total = data[0]?.total || 0;
          this.groupOwner = data[0]?.groupOwner || 0;
          this.groupMember = data[0]?.groupMember || 0;
          this.groupOwnerName = data[0]?.groupOwnerName || 0;
        });
      },
  
      // 删除操作
      async deleteBlog(userid, username) {
        const resConfirm = await this.$confirm(`此操作将永久删除【${username}】该用户,是否继续?`, '提示', {
        ...
        const { data: res } = await this.$http.get('http://api-vueblog.czc.cool/deleteGroupMember.php', { params: { id: userid, username: this.editForm.username } })
      ...
      editBlog() {
        this.$refs.editFormRef.validate(async (valid) => {
          ...
          const { data: res } = await this.$http.post('http://api-vueblog.czc.cool/deleteEdit.php', { editForm: this.editForm })
          if (res.result.status != 1) {
            // '更新失败！'
            return this.$message.error(res.result.mgs)
          }
