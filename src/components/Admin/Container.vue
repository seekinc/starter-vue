...
export default {
  ...
  methods: {
    ...
    getBlogList(number) {
      ...
      this.$http.get('http://api-vueblog.czc.cool/getUserBlogs.php', { params: this.queryInfo }).then((res) => {
        const { data } = res || {}
    ...
    async deleteBlog(BlogId, title) {
      ...
      const { data: res } = await this.$http.get('http://api-vueblog.czc.cool/deleteEdit.php', { params: { id: BlogId, username: this.editForm.username } })
